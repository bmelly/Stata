...
*syntax
		syntax varlist [if] [in] [pweight] [, BE FE RE Quantiles(numlist >0 <1 sort) Cluster(varname) qr_opts(string) BOOTstrap(string) Level(cilevel) noPrint Save_first(name) Load_first(name) n_small(integer 1) PARallel]
		if ("`be'"!="") + ("`fe'"!="") + ("`re'"!="") > 1 { 
