*! version 2.0.4 8mar2010 Blaise Melly
*  version 2.0.1 14sept2009 the bootstrap replications can be found in the mata matrix RQ_deco_BoOt
*  version 2.0.2 26jan2010 allow multicolinearity in some bootstrap draws
*  version 2.0.3 27jan2010, names of the columns of the matrices
*  version 2.0.4 8mar2010, correction: variables saved during bootstraping
...
*2.3 call the estimation command
		...
		mat colnames `results'=quantile fitted_1 fitted_X1C0 fitted_0 total_differential characteristics coefficients
		local nq=rowsof(`quants')
...
*2.4.b: bootstrap
	...
	*loop for each replication
			while `j'<=`reps'{
				...
				if (`rc'==0) {
					quietly drop _all
					mat `res'=r(results)
					mat `res1'=vec(`res'[1...,2])'
					quietly svmat `res1', names("X1_C1_")
					mat `res2'=vec(`res'[1...,3])'
					quietly svmat `res2', names("X1_C0_")
					mat `res3'=vec(`res'[1...,4])'
					quietly svmat `res3', names("X0_C0_")
					if( `j'==1){
	...
	* evaluate the boostrap results	
			...
			mat colnames `se'=se_total_differential se_characteristics se_coefficients
			use "`data'"
		}
	}
...
//calculate the effects of interest
	RQ_deco_ReT=RQ_deco_ReT,RQ_deco_ReT[.,2]-RQ_deco_ReT[.,4]
